...
// ── Progress checkboxes ──────────────────────────────────────────────
function sv(id, v) {
  localStorage.setItem(id, v ? '1' : '');
  updateProgress();
}
function loadChks(n) {
  document.querySelectorAll('#wb' + n + ' .dc').forEach(function(c) {
    if (localStorage.getItem(c.id) === '1') c.checked = true;
  });
}
function updateProgress() {
  var all = document.querySelectorAll('.dc').length;
  var done = 0;
  document.querySelectorAll('.dc').forEach(function(c) {
    if (localStorage.getItem(c.id) === '1') done++;
  });
  ...
  for (var w = 1; w <= 12; w++) {
    var wcs = document.querySelectorAll('[id^="c-w' + w + '-"]');
    var wdone = 0;
    wcs.forEach(function(c) { if (localStorage.getItem(c.id) === '1') wdone++; });
    var wp = document.getElementById('wp' + w);
    if (wp && wcs.length) wp.textContent = '(' + wdone + '/' + wcs.length + ')';
  }
}
function loadAllChks() {
...
function getTodayStudy() {
  ...
  var topics = [
    ...
    ['Sat','PROJECT DAY','Build something with this week\'s skills'],
    ['Sun','REST DAY','No study — recharge for Monday']
  ];
  var wk = Math.floor(diff / 7) + 1;
